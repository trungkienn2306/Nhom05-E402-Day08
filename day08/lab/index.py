...
def _split_by_size(
    ...
    while start < len(text):
        ...
        # Tìm ranh giới tự nhiên (dấu chấm câu hoặc xuống dòng) để tránh bị cắt giữa từ
        if end < len(text):
            # Lùi về tìm dấu cách hoặc xuống dòng gần nhất
            while end > start and text[end] not in ['.', '\n', ' ']:
                end -= 1
            if end == start:  # Nếu không tìm thấy, đành cắt cứng
                end = min(start + chunk_chars, len(text))
        ...
        # Tiến tới lấy overlap cho chunk tiếp theo
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    # 1. Khởi tạo Chroma DB Client
    chroma_client = chromadb.PersistentClient(path=str(db_dir))
    # Reset collection nếu chạy lại nhiều lần để tránh trùng lặp
    try:
    ...
    for filepath in doc_files:
        ...
        # 2. Xử lý và Cắt Chunk
        doc = preprocess_document(raw_text, str(filepath))
        chunks = chunk_document(doc)

        print(f"    → Đã tạo {len(chunks)} chunks. Đang embedding và lưu...")

        # 3. Vectorize & Lưu trữ vào DB
